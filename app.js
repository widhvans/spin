...
const backendUrl = "https://your-backend-api.com"; // Replace with your backend API URL

// Mock backend functions (replace with actual API calls)
async function fetchUserData(userId) {
    // Simulate fetching user data from MongoDB via backend
    return {
        user_id: userId,
        username: username,
        balance: 0,
...
async function performSpin(userId) {
    const rewards = [10, 20, 30, 50, 0];
    const weights = [0.4, 0.3, 0.2, 0.05, 0.05];
    const reward = rewards[Math.floor(Math.random() * rewards.length)]; // Replace with weighted random
    // Update backend (MongoDB) via API
    return { reward, spins_left: 2, balance: reward };
}
...
function updateUI(userData) {
    ...
    const referralLink = `https://t.me/YourBotUsername?start=${referral_code}`;
...
document.getElementById("copy-referral").addEventListener("click", async () => {
    const userData = await fetchUserData(userId);
    const referralLink = `https://t.me/YourBotUsername?start=${userData.referral_code}`;
